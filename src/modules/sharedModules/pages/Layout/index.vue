    ...
    <main class="app-wrapper">
        ...
        <Transition name="mobilemenu">
            <mobile-sidebar
                ...
                v-if="
                    window.width < 1280 &&
                    this.$store.themeSettingsStore.mobielSidebar
                "
            />
        </Transition>
        ...
        >
            <div
                class="page-content"
                :class="
                    this.$route.meta.appheight ? 'h-full' : 'page-min-height'
                "
            >
                <div
        ...
        />
        <FunctionSidebar v-if="displaySidebar" />
    </main>
</template>
<script>
...
import { sideBarItem } from "@/modules/sharedModules/data/index";
import FunctionSidebar from "@/modules/sharedModules/pages/Layout/FunctionSidebar.vue";
import Sidebar from "@/components/Sidebar/index.vue";
import window from "@/mixins/window";
import MobileSidebar from "@/components/Sidebar/MobileSidebar.vue";
import FooterMenu from "@/components/Footer/FooterMenu.vue";
import { useThemeSettingsStore } from "@/store/themeSettings";
import { useAppStore } from "@/modules/sharedModules/store/index.ts";

export default {
    created() {
        this.$store.themeSettingsStore = useThemeSettingsStore();
        this.$store.app = useAppStore();
    },
    mixins: [window],
    components: {
        Header,
    ...
    computed: {
        displaySidebar() {
            console.log(this.$store.app.$state.functionSidebar.display);
            return this.$store.app.$state.functionSidebar.display;
        },
    },
    watch: {
        // $route
